...
public class ChatRepository {
        ...
        return query.addSnapshotListener((snapshots, e) -> {
            ...
            for (var doc : snapshots.getDocuments()) {
                try {
                    ...
                    if (msg != null) {
                        if (msg.getMessageId() == null || msg.getMessageId().isEmpty()) msg.setMessageId(doc.getId());
                        messages.add(msg);
                    }
                } catch (Exception ex) { Log.e(TAG, "Error parsing message: " + doc.getId(), ex); }
            }
    ...
    public ListenerRegistration listenChatRooms(String userId, ChatRoomsListener listener) {
        ...
        Filter eqFilter = Filter.or(
                Filter.equalTo("user1Id", userId), Filter.equalTo("user2Id", userId),
                Filter.equalTo("userId1", userId), Filter.equalTo("userId2", userId)
        );

        String[] collections = {"chat_rooms", "chats"};
        for (String coll : collections) {
            regs.add(db.collection(coll).where(eqFilter).addSnapshotListener((snap, e) -> {
                if (snap != null) {
                    for (var doc : snap.getDocuments()) {
                        try {
                            ChatRoom r = doc.toObject(ChatRoom.class);
                            if (r != null) { r.setRoomId(doc.getId()); roomsMap.put(doc.getId(), r); }
                        } catch (Exception ex) { Log.e(TAG, "Error deserializing ChatRoom", ex); }
                    }
                    notify.run();
                }
            }));
    ...
    public void markMessagesAsRead(String chatId, String otherUserId, FirebaseCallback callback) {
        db.collection("chats").document(chatId).collection("messages")
                .whereEqualTo("senderId", otherUserId).whereEqualTo("isRead", false).get()
                .addOnSuccessListener(query -> {
                    if (query.isEmpty()) { if (callback != null) callback.onSuccess(); return; }
                ...
                .addSnapshotListener((snapshots, e) -> {
                    ...
                    for (var doc : snapshots.getDocumentChanges()) {
                        if (doc.getType() == com.google.firebase.firestore.DocumentChange.Type.MODIFIED) {
                            ...
                            if (isRead != null && listener != null) listener.onMessageReadStatusChanged(messageId, isRead);
                        }
                    }
                });
    }
    ...
    public ListenerRegistration listenToTyping(String chatId, TypingListener listener) {
        return db.collection("chats").document(chatId).addSnapshotListener((snapshot, e) -> {
    ...
    public interface FirebaseCallback { void onSuccess(); void onFailure(Exception error); }
    public interface MessageListener { void onMessages(List<ChatMessage> messages); void onError(Exception error); }
    public interface ChatRoomsListener { void onChatRooms(List<ChatRoom> rooms); void onError(Exception error); }
    public interface ReadReceiptListener { void onMessageReadStatusChanged(String messageId, boolean isRead); void onError(Exception error); }
    public interface TypingListener { void onTypingChanged(Map<String, Boolean> typingStates); void onError(Exception error); }
}
